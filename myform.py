...
def проверка_вопрос(вопрос: str):
    ...
    if len(вопрос) < 10:
        raise HTTPError(status=401, body="Вопрос должен быть не менее 10 символов.")
    elif len(вопрос) > 100:
        raise HTTPError(status=401, body="Вопрос должен быть не более 100 символов.")
...
def проверка_имя(имя: str):
    ...
    if not шаблон_имя.match(имя):
        raise HTTPError(status=401, body="Невалидное имя. Введите валидное имя.")

    if len(имя) < 3:
        raise HTTPError(status=401, body="Имя должно быть не менее 3 символов.")

    if len(имя) > 20:
        raise HTTPError(status=401, body="Имя должно быть не более 20 символов.")
...
def проверка_email(email: str):
    ...
    if len(email) > 50:
        raise HTTPError(status=401, body="Невалидный email. Введите валидный email.")
    
    pattern_email = re.compile(r'^[a-zA-Z0-9_-]+@[a-zA-Z0-9.-]+\.[a-zA-Z]{2,}$')
